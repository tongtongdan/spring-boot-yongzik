...
public class AtricleController {
    ...
    @PostMapping()
    public ResponseEntity<ApiResponse<ArticleDetailResponse>> addArticle(@RequestBody
    ...
    public ResponseEntity<ApiResponse<ArticleDetailResponse>> getOneArticle(@PathVariable Long id){
        ...
        return ResponseEntity.ok(ApiResponse.success(200,"게시물 개별 조회에 성공하였습니다.",
                articleResponse));
    }

    @GetMapping()
    public ResponseEntity<ApiResponse<List<ArticleSummaryResponse>>> getArticle(){
        List<ArticleSummaryResponse> articleAllResponse = articleService.getArticles();
        return ResponseEntity.ok(ApiResponse.success(200, "게시물 전체 조회에 성공하였습니다.", articleAllResponse));
    }
    ...
    public ResponseEntity<ApiResponse<Void>> deleteArticle(@PathVariable Long id){
        ...
        return ResponseEntity.ok(ApiResponse.success(204,"게시물을 삭제하는데 성공하였습니다."));
    }
}
